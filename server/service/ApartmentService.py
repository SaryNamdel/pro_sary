...
class ApartmentService:
    ...
    def _coerce(self, dto):
        """
        מאפשר לקבל גם dict מהבקשה ולגשת אליו עם getattr כמו DTO.
        """
    ...
    def _validate_new(self, dto):
        """
        וולידציה לפריט חדש. תיתמך גם בקלט dict.
        """
        dto = self._coerce(dto)

        if not hasattr(dto, "cityId") or dto.cityId is None:
            raise MissingFieldException("cityId")

        if hasattr(self.repo, "exists_by_city") and callable(getattr(self.repo, "exists_by_city")):
            if self.repo.exists_by_city(dto.cityId):
                raise ItemAlreadyExistsException(f"cityId={dto.cityId}")

    def _from_dto(self, dto) -> Apartment:
        """
        בונה אובייקט ORM מתוך DTO/Body. תיתמך גם בקלט dict.
        """
        dto = self._coerce(dto)
        return Apartment(
            apartmentId=getattr(dto, "apartmentId", None),
